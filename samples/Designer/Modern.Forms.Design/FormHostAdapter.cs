...
{
    // Unlike System.Windows.Forms.Form, Modern.Forms.Form does not inherit Control,
    // so it cannot be added directly to a designer surface.  This adapter is needed
    // to host the Form in a Control.
    public class FormHostAdapter : Control
    {
        private DocumentDesigner designer;
        private Form form;
        private ControlDesigner3? selected_control;
        ...
        public FormHostAdapter (DocumentDesigner designer)
        {
            Dock = DockStyle.Fill;

            this.designer = designer;
            this.form = designer.Form;

            foreach (var control in form.Controls.ToArray ()) {
                if (control is not ControlDesigner3) {
                    form.Controls.Remove (control);
                    var new_control = new ControlDesigner3 (control);
        ...
        protected override void OnMouseDown (MouseEventArgs e)
        {
            ...
            if (control is not null) {
                ...
                if (host.GetDesigner (control) is ControlDesigner cd) {
                    cd.OnMouseDragBegin (e.X - 15, e.Y - 15);
                    return;
                }
            }
        ...
        //    if (GetControlAtLocation (e.Location) is ControlDesigner3 c) {
        //        selected_control = c;
